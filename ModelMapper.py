...
def run():
    ...
    rule.add_property(name='Effect', value='Permit')
    rule.add_property(name='RuleId', value='NewTicket')
    rule.add_target(target=target)

    policy.add_property(name='xmlns', value='urn:oasis:names:tc:xacml:3.0:core:schema:wd-17')
    policy.add_property(name='PolicyId', value='NewTicketOnlySupport')
    policy.add_property(name='RuleCombiningAlgId', value='urn:oasis:names:tc:xacml:3.0:rule-combining-algorithm:deny-unless-permit')
    policy.add_property(name='Version', value='1.0')
